...
function usage()
{
    ...
    echo "$0 <project_name> (c|java|python)*"
    echo "override=install allow to install toolbox over an existing project"
}
...
function setup_initial_git()
{
    if [[ -d .git ]]
    then
	echo "[ERROR] .git found, won't rinit git project" >&2
	return
    else
	echo "[INFO] create an empty git project here $(pwd)"
    fi
    git init
    git add ./init.sh
    for l in $languages
...
if [[ $override == "install" ]]
then
    echo "[WARNING] override=$ovveride used, project $project_name should already exists in $(pwd)" >&2
else
    if [[ -d $project_name ]]
    then    
...
if [[ ! -d \$ARTLOG_TOOLBOX ]]
then
    git clone ${ARTLOG_GIT_REPOSITORY} \$ARTLOG_TOOLBOX
    if [[ -d \$ARTLOG_TOOLBOX ]]
    then
...
if [[ $override == "install" ]]
then
    echo "[WARNING] no initial git setup in override mode $override"
else
    setup_initial_git
fi
