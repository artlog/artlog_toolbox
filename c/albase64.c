...
void setup_func_charto6bits(char (*func_6bitstochar)(unsigned int), unsigned char *charto6bits)
{
  for (int i=0; i< 128; i++)
    {
      charto6bits[i]=65;
    }
  charto6bits[const_complement]=64;
  char c = 0;
...
void setup_charto6bits(struct albase64_context * albase64_context_p)
{
#ifdef DEBUG_BASE64
  aldebug_printf(DBGSTREAM,"setup_charto6bits(flags=%i,%s)\n",albase64_context_p->flags,albase64_context_p->base64chars);
#endif
  if ( albase64_context_p->flags == 0 ) {
    albase64_context_p->flags|=1;
    char * charto6bits = albase64_context_p->charto6bits;
    for (int i=0; i< 128; i++) {
      charto6bits[i]=65;
    }
    charto6bits[albase64_context_p->complement]=64;
    char c = 0;
    for (unsigned char i=0; i<= _6BITSMASK; i++) {
      c=_CTX_2_6BITSTOCHAR(albase64_context_p,i);
#ifdef DEBUG_BASE64
      aldebug_printf(DBGSTREAM,"%i=%c,",i,c);
#endif
      if ( c >= 0 ) {
	charto6bits[c]=i;
      }
...
{
  return ALBASE64_CONTEXT_DEFAULT.base64chars[inbits & _6BITSMASK];
}

// read 6 bits
char albase64url_6bitstochar(unsigned int inbits)
{
  return _CTX_2_6BITSTOCHAR((&ALBASE64_CONTEXT_URL),inbits);
}
...
int albase64func_frominput(char (*func_6bitstochar)(unsigned int) , struct alinputstream * inputstream, struct aloutputstream * output)
{
  ...
  if ( ( pad > 0 ) && ( pad <=  16 ) )
    {
      // 9..16 => complement ('=')
      aloutputwritechar(output,const_complement);
      if ( pad <= 8 )
	{
...
int albase64_encode(struct albase64_context * albase64_context_p , struct alinputstream * inputstream, struct aloutputstream * output)
{
  ...
  do {
    ...
    if ( bitreadden > 0 )
      {
	bits+=bitreadden;
	b64char = _CTX_2_6BITSTOCHAR( albase64_context_p, block);
#ifdef DEBUG_BASE64
	aldebug_printf(DBGSTREAM,"b64char '%c'(%x) block %08x bit readden %i\n",b64char,b64char,block,bitreadden);
#endif
	// addchar to output
  ...
  // pad input 6 bits * 4
  int pad = bits % 24;
  char complement = albase64_context_p->complement;
...
int albase64_decode_tobitfieldwriter( unsigned char * charto6bits, struct alinputstream * inputstream, struct bitfieldwriter * bfoutput_p)
{
  int bits = 0;
  // bytes
  int read = 0;
...
int albase64func_decode_frominput(char (*func_6bitstochar)(unsigned int) , struct alinputstream * inputstream, struct aloutputstream * output)
{
  ...
  setup_func_charto6bits(func_6bitstochar,charto6bits);
  
  return albase64_decode_tobitfieldwriter(charto6bits, inputstream, &bfoutput);
}
...
int albase64_decode(struct albase64_context * albase64_context_p , struct alinputstream * inputstream, struct aloutputstream * output)
{
  ...
  // build reverse table for char to 6bits
  setup_charto6bits(albase64_context_p);
  
  return albase64_decode_tobitfieldwriter(albase64_context_p->charto6bits, inputstream, &bfoutput);
}
...
int albase64(aldatablock * input, struct aloutputstream * output)
{
  ...
  return albase64_encode(&ALBASE64_CONTEXT_DEFAULT,&inputstream, output);
}
